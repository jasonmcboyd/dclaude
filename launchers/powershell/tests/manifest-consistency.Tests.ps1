Describe 'Module manifest consistency' {
    ...
    It 'CI manifest exports the same functions as the dev manifest' {
        $devManifest = Import-PowerShellDataFile -Path "$PSScriptRoot/../dclaude.psd1"
    ...
    It 'manifest FunctionsToExport matches files in Public/' {
        $devManifest = Import-PowerShellDataFile -Path "$PSScriptRoot/../dclaude.psd1"
        $devFunctions = $devManifest.FunctionsToExport | Sort-Object

        $publicFiles = Get-ChildItem "$PSScriptRoot/../Public" -Filter '*.ps1' |
