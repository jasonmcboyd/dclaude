...
launchers/
  powershell/                   # the PowerShell launcher (PSGallery artifact: dclaude)
    ...
    bin/                        #   compiled Go entrypoint binaries (dclaude-entrypoint-<os>-<arch>[.bin.gz]); built by CI, gitignored
sidecars/
...
- Go entrypoint binary → the platform-appropriate binary from `bin/` is mounted read-only at launch. Windows binaries ship gzip-compressed (`.bin.gz`) so Defender's ML heuristic can't scan the PE header during PowerShellGet extraction through `%TEMP%`. On Windows the launcher stages the `.gz` (without decompressing) to a content-hashed `%LOCALAPPDATA%\dclaude\.bin\<hash>\` directory alongside a `bootstrap.ps1` script, and mounts that directory (Windows can't bind-mount a single file; staging also avoids OneDrive reparse points). The bootstrap script decompresses the `.gz` inside the container's isolated filesystem (where host-side Defender can't reach) and execs the binary. Dev override: `DCLAUDE_ENTRYPOINT_SRC` points at a locally built binary (raw, not gzipped); the bootstrap handles both cases.
...
The Go binary (`dclaude-entrypoint-<os>-<arch>[.bin.gz]`) is the sole container entrypoint. It is built from the `entrypoint/` Go module, shipped inside the PowerShell module under `bin/`, and mounted read-only into the container at launch. There are no shell entrypoints. The bootstrap sequence is:
