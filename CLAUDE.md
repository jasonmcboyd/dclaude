...
- Go entrypoint binary → the platform-appropriate binary from `bin/` is mounted read-only at launch. Windows binaries ship gzip-compressed (`.bin.gz`) so Defender's ML heuristic can't scan the PE header during PowerShellGet extraction through `%TEMP%`. On Windows the launcher stages the binary to a content-hashed `%LOCALAPPDATA%\dclaude\.bin\<hash>\` directory, decompressing the `.gz` to `.exe` during staging, and mounts that directory (Windows can't bind-mount a single file; staging also avoids OneDrive reparse points and the running-executable lock). Dev override: `DCLAUDE_ENTRYPOINT_SRC` points at a locally built binary (raw, not gzipped).
